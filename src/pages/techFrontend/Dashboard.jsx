...
import { Wrench, Search, LayoutGrid, CheckCircle2, AlertCircle, LogOut } from 'lucide-react';
...
function Dashboard() {
  ...
  // --- States ---
  const [assets, setAssets] = useState([]); // Assets ka data store karne ke liye simple state
  const [searchQuery, setSearchQuery] = useState('');
  const [isLoading, setIsLoading] = useState(true);

  // --- 1. Firebase Se Assets Load Karna ---
  useEffect(() => {
    const loadAssets = async () => {
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          // Agar Firebase ka timestamp object hai to use ISO string me convert karein
        ...
        setAssets(tempAssets); // State mein data save kar diya
      } catch (error) {
  ...
  // --- 3. Search Filtering (Dhoondhne ka logic) ---
  ...
  // --- 4. Metrics Counters (Upar ke dabbe/stats) ---
  ...
  <LogOut size={16} className="ml-1 opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          ...
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-5 flex items-center gap-4">
            <div className="p-3 bg-indigo-500/10 border border-indigo-500/20 text-[#4CC9F0] rounded-xl">
              <LayoutGrid size={20} />
            </div>
            <div>
